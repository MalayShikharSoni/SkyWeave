<x-app-layout>
    @section('title', 'Edit NAVAID — ' . $navaid->identifier)
    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-white">Edit NAVAID</h1>
                <p class="text-sm text-sky-400 mt-1">
                    Modifying <span class="font-mono text-accent-300">{{ $navaid->identifier }}</span>
                    <span class="text-sky-600">—</span> {{ $navaid->name }}
                </p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="glass-card p-8">
                <form method="POST" action="{{ route('navaids.update', $navaid) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                        <div>
                            ...
                            <input type="text" id="identifier" name="identifier"
                                   value="{{ old('identifier', $navaid->identifier) }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                        <div>
                            ...
                            <select id="type" name="type" class="form-input-dark" required>
                                @foreach (\App\Models\Navaid::TYPES as $t)
                                    <option value="{{ $t }}" {{ old('type', $navaid->type) == $t ? 'selected' : '' }}>
                                        {{ str_replace('_', '/', $t) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" id="frequency" name="frequency"
                                   value="{{ old('frequency', $navaid->frequency) }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-8">
                        <div>
                            ...
                            <input type="number" id="latitude" name="latitude"
                                   value="{{ old('latitude', $navaid->latitude) }}"
                        ...
                        <div>
                            ...
                            <input type="number" id="longitude" name="longitude"
                                   value="{{ old('longitude', $navaid->longitude) }}"
